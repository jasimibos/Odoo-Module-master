...
class ResUser(models.Model):
    ...
    def set_read_only_user(self):
        ...
        read_only_grp_id = self.env['ir.model.data']._xmlid_lookup('iBOS_read_only_user_app.group_read_only_user')[2]
...
class IrModelAccess(models.Model):
    ...
    def check(self, model, mode='read', raise_exception=True):
        ...
        if self.env.user.has_group('iBOS_read_only_user_app.group_read_only_user'):
            print(mode, self._name, model)
            if model == 'stock.move.line':
                print("Ok data come from Stock Move Line")
                mode = 'write'
        ...
        if model_name not in obj_list:
            if self.env.user.has_group('iBOS_read_only_user_app.group_read_only_user'):
                ...
                if model_name == "stock.picking":
                    print(model_name)
                elif model_name == "stock.move.line":
                    print(model_name)
                elif mode != 'read':
