...
class PosDetails(models.TransientModel):
    ...
    def _default_start_date(self):
        ...
        date_ = latest_start_dates[0].date()
        new_date_time = [datetime.combine(date_, datetime.min.time())]
        return new_date_time and min(new_date_time) or fields.Datetime.now()

    start_date = fields.Datetime(required=True, default=_default_start_date)
    end_date = fields.Datetime(required=True, default=fields.Datetime.now())
    pos_config_ids = fields.Many2many('pos.config', 'pos_detail_confi',
    ...
    def generate_report(self):
        ...
        return self.env.ref('ibos_pos_sales_report.pos_sale_details_report').report_action([], data=data)
